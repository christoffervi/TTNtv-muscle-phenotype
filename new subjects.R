...
dat_df %>% pivot_longer(c(contains("_FF")
                          )) %>% 
  mutate(group = factor(Gruppe)) %>% 
  lme4::lmer(value~group+(1|name)+rms::rcs(BMI,4)+rms::rcs(Age,4)+Sex, data = .,
             REML =T) %>% 
  summary()

dat_df %>% pivot_longer(c(thigh_FF, calf_FF, back_FF, contains("_FF"))) %>% 
  mutate(group = factor(Gruppe)) %>% 
  lm(value~group+rms::rcs(BMI,3)+rms::rcs(Age,3)+Sex+name, data = .) %>% 
  summary() %>% broom::tidy(conf.int=T)
...
dat_df %>% select(ends_with("FF"), BMI, Gruppe, Age,Sex) %>% 
  ...
  mutate(Gruppe = factor(Gruppe)) %>% 
  pivot_longer(ends_with("_FF"), names_to = 'muscle', values_to = 'FF') %>% 
...
model <- 
  ...
  lme4::lmer(FF~Gruppe+Age+BMI+Sex+(1|ID)+(1|muscle),data = . )
